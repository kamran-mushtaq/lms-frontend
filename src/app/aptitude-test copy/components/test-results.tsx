...
export function TestResults({ results, assessment, userResponses, onReturn }: TestResultsProps) {
  ...
  // When results are displayed, clear the aptitude test redirect timestamp
  // to ensure future redirects work correctly
  useEffect(() => {
    localStorage.removeItem('aptitude_redirect_timestamp');
    // Also clear any current aptitude test ID to prevent resuming the test
    localStorage.removeItem('current_aptitude_test_id');
    
    // Log successful completion for debugging purposes
    console.log("Aptitude test completed with results:", results);
    localStorage.setItem('aptitude_test_completion', JSON.stringify({
  ...
  const handleGoToDashboard = () => {
    // Clear any redirect prevention
    localStorage.removeItem('aptitude_redirect_timestamp');
    
    // Navigate directly instead of using the onReturn callback
    // This ensures a fresh load of the dashboard with the updated enrollment status
                  ...
                  <Card className="bg-green-50 border-green-200">
                    ...
                    <CardContent>
                      <p className="text-green-800 mb-4">
                        Congratulations on passing your aptitude test! Here's what you can expect next:
                      </p>
                      <ul className="space-y-2 text-sm text-green-800">
                        <li className="flex gap-2">
                          <span>•</span>
                          <span>You now have full access to this subject's course materials</span>
                        </li>
                        <li className="flex gap-2">
                          <span>•</span>
                          <span>Your progress will be tracked as you work through the materials</span>
                        </li>
                        <li className="flex gap-2">
                          <span>•</span>
                          <span>If you have other subjects that need aptitude tests, return to the dashboard to take them</span>
                        </li>
                        <li className="flex gap-2">
                          <span>•</span>
                          <span>Begin your learning journey by exploring the available chapters and lectures</span>
                        </li>
                      </ul>
                    </CardContent>
                  </Card>
